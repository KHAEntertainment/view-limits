...
const READINESS_ITEMS = Object.freeze([
  { id: 1, item: 'caller-identity', verdict: 'partial', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 1' },
  { id: 2, item: 'effective-model-verified', verdict: 'blocking', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 2' },
  { id: 3, item: 'selected-profile-account', verdict: 'blocking', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 3' },
  { id: 4, item: 'native-resource-apis', verdict: 'partial', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 4' },
  { id: 5, item: 'provider-adapters-composed', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 5' },
  { id: 6, item: 'identity-separation', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 6' },
  { id: 7, item: 'cache-only-live-modes', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 7' },
  { id: 8, item: 'gate-network-free', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 8' },
  { id: 9, item: 'capability-registry', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 9' },
  { id: 10, item: 'deterministic-eligibility', verdict: 'pass', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 10' },
  { id: 11, item: 'semantic-inputs-sufficient', verdict: 'blocking', evidence: 'docs/jev-readiness.md — eleven-item gate table, item 11' },
]);

// The required item set — the ids the gate needs evidence for. An evidence
// array that does not cover every required id is treated as missing
// prerequisites, never as a complete checklist.
const REQUIRED_ITEM_IDS = Object.freeze(READINESS_ITEMS.map((i) => i.id));
const REQUIRED_ITEM_NAMES = new Map(READINESS_ITEMS.map((i) => [i.id, i.item]));
...
const READINESS_REASONS = Object.freeze({
  ITEM_ABSENT: 'readiness-item-absent',
});
...
//
// Fail-closed in BOTH directions: any item whose verdict is not 'pass' keeps
// the gate closed, and any REQUIRED id absent from the supplied set is
// synthesized as { verdict:'blocking', reason:'readiness-item-absent' } — a
// torn-out checklist can never read as complete. An empty items array
// therefore closes the gate on all eleven items.
//
...
// the gate closed (partial, blocking, and absent alike).
function evaluateReadiness({ items, modelVerified } = {}) {
  ...
  for (const raw of src) {
    ...
    if (n.id !== null && REQUIRED_ITEM_NAMES.has(n.id)) {
      // First occurrence wins — deterministic given stable input order.
      if (!byId.has(n.id)) byId.set(n.id, n);
    } else {
      extras.push(n);
    }
  }
  const normalized = REQUIRED_ITEM_IDS.map((id) => byId.get(id) || {
    id,
    item: REQUIRED_ITEM_NAMES.get(id),
    verdict: 'blocking',
    evidence: null,
    reason: READINESS_REASONS.ITEM_ABSENT,
  });
  for (const extra of extras) normalized.push(extra);
  normalized.sort((a, b) => ((a.id === null ? Infinity : a.id) - (b.id === null ? Infinity : b.id)) ||
...
module.exports = { READINESS_ITEMS, VERDICTS, READINESS_REASONS, evaluateReadiness };

...
//
// `refresh: true` adds bounded live reads through the supported Traycer CLI
// adapter (lib/traycer-adapter.js) — only when a Traycer caller identity can
// be formed; otherwise the request is recorded in requestedRefresh and the
// result stays cache-only with a live-reads-unavailable diagnostic. Live rows
// merge with cached rows by composite key (a live row replaces the same-key
// cached row; cache-only rows survive untouched). Per-source failures become
// scoped diagnostics and unknown facts — never exceptions.
//
...
// identity), while cache-only rows survive untouched. Cached order is
// preserved; new live rows append in adapter order.
...
async function getRuntimeSnapshot({
  ...
  // Live reads run only on explicit refresh; refresh:false never reaches the
  // adapter (zero Traycer CLI/RPC/subprocess invocations). The adapter owns
  // all Traycer command syntax; a thrown adapter error degrades to a scoped
  // diagnostic rather than rejecting the snapshot.
  let live = null;
  if (refresh) {
    const readLive = io.readTraycer || traycerAdapter.readTraycerRuntime;
    try {
      live = await readLive({
        callerContext,
      ...
      live = null;
      diagnostics.push(diag('traycer-read-failed', 'traycer'));
    }
    if (live && Array.isArray(live.diagnostics)) {
      for (const d of live.diagnostics) {
        if (isPlainObject(d) && nonEmptyString(d.code)) {
          diagnostics.push({ code: d.code, scope: nonEmptyString(d.scope) ? d.scope : null, summary: nonEmptyString(d.summary) ? d.summary : (DIAG_SUMMARIES[d.code] || null) });
        }
      }
    }
  }
  ...
  const snapshot = {
    ...
    completeness: 'empty',
    diagnostics,
    caller: assembleCaller(callerContext, live && live.caller),
    sessions: mergeSectionRows(sidecarDoc.sessions, live && live.sessions, SECTION_KEYS.sessions),
    harnesses: mergeSectionRows(sidecarDoc.harnesses, live && live.harnesses, SECTION_KEYS.harnesses),
    profiles: mergeSectionRows(sidecarDoc.profiles, live && live.profiles, SECTION_KEYS.profiles),
    routes,
  };

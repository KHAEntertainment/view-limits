...
const { factIsKnown, TASK_PROFILE_REASONS } = require('./task-profile');
const { JEV_REASONS } = require('./jev-client');
const { READINESS_REASONS } = require('./jev-readiness');
const { RECEIPT_REASONS } = require('./receipts');
...
const REASON_SUMMARIES = {
  // scoring dimension — unresolved codes
  ...
  // scoring dimension — evidence codes
  ...
  // jev-client.js — network/classification reason codes
  'jev-model-unconfigured': 'no Jev model is configured; classification cannot proceed.',
  'jev-endpoint-unconfigured': 'no Jev endpoint is configured; classification cannot proceed.',
  'jev-transport-absent': 'no Jev request transport was injected; classification cannot proceed.',
  'jev-catalog-transport-absent': 'no catalog fetch transport was injected; model verification cannot proceed.',
  'jev-timeout': 'the Jev classification request timed out.',
  'jev-unreachable': 'the Jev classification endpoint was unreachable.',
  'jev-http-error': 'the Jev endpoint returned a non-2xx HTTP status.',
  'jev-catalog-unreachable': 'the model catalog endpoint was unreachable.',
  'jev-catalog-timeout': 'the model catalog request timed out.',
  'jev-catalog-malformed': 'the model catalog response was not a valid catalog document.',
  'jev-model-not-in-catalog': 'the configured model id was not found in the provider catalog.',
  'jev-structured-output-unverified': 'the configured model does not advertise structured output support in the catalog.',
  'jev-response-malformed': 'the Jev classification response was not a valid JSON document.',
  'jev-response-schema-invalid': 'the Jev classification response failed schema validation.',
  'jev-low-confidence': 'the Jev classifier returned a confidence score below the configured floor.',
  // task-profile.js — profile dimension reason codes
  'task-profile-field-absent': 'a required task profile dimension field was not supplied.',
  'task-profile-value-invalid': 'a task profile dimension value is outside the allowed vocabulary.',
  // task-profile.js — classifier response validation codes
  'response-not-object': 'the classifier response is not a JSON object.',
  'confidence-invalid': 'the classifier confidence is not a finite number in [0, 1].',
  'profile-not-object': 'the classifier profile is not a JSON object.',
  // recommend.js — recommendation pipeline codes
  'jev-readiness-gate-closed': 'the Jev readiness gate is closed; the Jev network path is dormant.',
  'jev-model-unverified': 'all readiness items pass but the configured model’s structured-output support is not verified.',
  'jev-verification-not-attempted': 'readiness items did not all pass, so model verification was not attempted.',
  'profile-dimension-unresolved': 'a task profile dimension could not be resolved from available evidence.',
  'jev-suggestion-unknown-candidate': 'the Jev classifier suggested a candidate id that does not appear in the candidate list.',
  'jev-suggestion-ineligible': 'the Jev classifier suggested a candidate that is ineligible under hard eligibility.',
  'candidate-id-duplicate': 'two or more candidates share the same id; attribution is ambiguous, so the id is rejected.',
  // score-candidates.js — additional fallback codes
  'cached-state-absent': 'no cached observation exists for the candidate route state.',
  'caller-fact-absent': 'a required caller identity fact was not available.',
  'candidate-fact-absent': 'a required candidate fact was not available.',
  // frozen-module fact-reason codes (lib/traycer-adapter.js, lib/runtime-snapshot.js)
  'traycer-field-absent': 'a required field from the Traycer CLI was not present in the response.',
  'traycer-field-unconforming': 'a field from the Traycer CLI did not match the expected type or shape.',
  'availability-pending': 'the harness availability probe has not yet completed.',
  'harness-catalog-absent': 'the harness does not appear in the Traycer harness catalog.',
  'native-usage-unobserved': 'no native usage data was observed for this profile.',
  'no-cached-observation': 'no cached observation exists for this route in the status cache.',
  'cached-field-absent': 'a required field is missing from the cached route entry.',
  'model-evidence-absent': 'no model evidence is available from the cached route entry.',
  'model-comparison-unavailable': 'the effective vs default model comparison cannot be performed.',
  // task-profile.js — additional classifier-response validation codes
  'suggestedCandidateId-invalid': 'the classifier suggestedCandidateId is present but not a non-empty string.',
  // jev-readiness.js
  'readiness-item-absent': 'a required readiness item was absent from the supplied evidence.',
  // receipts.js
  'receipt-absent': 'no credential-submission receipt exists, or it has expired.',
  // frozen-module fact-reason codes — extended curated coverage
  'cached-field-invalid': 'a cached route field is present but does not conform to the expected shape.',
  'config-field-absent': 'a required field is missing from the configured route.',
  'route-not-configured': 'the route does not appear in the configured routes.',
  'registry-name-absent': 'no registry name was supplied for the lookup.',
  'registry-model-absent': 'the model is not registered in the capability registry.',
  'registry-harness-absent': 'the harness is not registered in the capability registry.',
  'registry-family-absent': 'the family is not registered in the capability registry.',
};

// Dynamic profile.*-invalid codes (task-profile.js generates these per dimension).
for (const dim of ['taskKind', 'exploration', 'specification', 'reasoning', 'risk', 'autonomy', 'verification', 'executionStyle']) {
  REASON_SUMMARIES[`profile.${dim}-invalid`] = `the classifier profile dimension '${dim}' is missing or not in the allowed vocabulary.`;
}

// All declared reason codes across the coupled modules, for the
// completeness test — built from the live registries each module exports,
// plus the curated frozen snapshot. Resolved lazily because recommend.js
// requires this module: a top-level require of recommend here would create
// a load cycle and observe partial exports.
let declaredReasonsCache = null;
function allDeclaredReasons() {
  if (!declaredReasonsCache) {
    const { RECOMMEND_REASONS } = require('./recommend');
    declaredReasonsCache = Object.freeze([...new Set([
      ...REASON_ORDER,
      ...Object.values(SCORE_FALLBACK_REASONS),
      ...JEV_REASONS,
      ...Object.values(TASK_PROFILE_REASONS),
      ...Object.values(RECOMMEND_REASONS),
      ...Object.values(READINESS_REASONS),
      ...Object.values(RECEIPT_REASONS),
      ...CURATED_FROZEN_REASON_CODES,
    ])]);
  }
  return declaredReasonsCache;
}

// Fallback reason literals this module emits when an upstream fact carries
// no reason of its own — kept in a registry so the completeness test stays
// coupled to what the code actually emits.
const SCORE_FALLBACK_REASONS = Object.freeze({
  CACHED_STATE_ABSENT: 'cached-state-absent',
  CALLER_FACT_ABSENT: 'caller-fact-absent',
  CANDIDATE_FACT_ABSENT: 'candidate-fact-absent',
});

// Frozen-module fact-reason codes — a hand-maintained snapshot. Frozen
// modules cannot export registries, so a new emitted literal there reaches
// this list only through review. Diagnostic codes are excluded on purpose:
// runtime-snapshot.js and runtime-sidecar.js self-summarize via
// DIAG_SUMMARIES, and eligibility.js self-summarizes its verdict reasons
// internally — neither indexes REASON_SUMMARIES.
const CURATED_FROZEN_REASON_CODES = Object.freeze([
  // lib/traycer-adapter.js fact reasons
  'traycer-field-absent', 'traycer-field-unconforming',
  'availability-pending', 'harness-catalog-absent', 'native-usage-unobserved',
  // lib/runtime-snapshot.js fact reasons
  'no-cached-observation', 'cached-field-absent', 'cached-field-invalid',
  'cached-state-absent', 'caller-fact-absent', 'config-field-absent',
  'model-evidence-absent', 'model-comparison-unavailable', 'route-not-configured',
  // lib/capability-registry.js fact reasons
  'registry-name-absent', 'registry-model-absent',
  'registry-harness-absent', 'registry-family-absent',
]);
...
function scoreTaskFit({ profile, reg, modelFact, modelEntryFact, declaredTaskCaps }) {
  ...
  if (kind === null) {
    const why = isPlainObject(kindFact) && nonEmptyString(kindFact.reason)
      ? kindFact.reason : TASK_PROFILE_REASONS.FIELD_ABSENT;
...
function scoreHarnessAffinity({ reg, harnessFact, callerHarnessFact }) {
  ...
  if (callerHarness === null) {
    const why = nonEmptyString(callerHarnessFact.reason) ? callerHarnessFact.reason : SCORE_FALLBACK_REASONS.CALLER_FACT_ABSENT;
  ...
  if (candHarness === null) {
    const why = nonEmptyString(harnessFact.reason) ? harnessFact.reason : SCORE_FALLBACK_REASONS.CANDIDATE_FACT_ABSENT;
...
function scoreResourcePressure({ routeIdFact, routeStateFact, routeFreshFact, nowMs }) {
  ...
  if (!factIsKnown(routeStateFact)) {
    const why = nonEmptyString(routeStateFact.reason) ? routeStateFact.reason : SCORE_FALLBACK_REASONS.CACHED_STATE_ABSENT;
...
function scoreCostSpeed({ preferences, costFact, speedFact }) {
  ...
  if (wantLowCost) {
    ...
    // Own-property check only: `in` would read the prototype chain and admit
    // values like 'constructor'/'toString', pushing a non-number into the
    // contribution and breaking the total ordering.
    if (tier === null || !Object.hasOwn(COST_TIERS, tier)) {
      reasons.push(reason('cost-signal-unproven', 'candidate'));
      unresolved = true;
    } else {
  ...
  if (wantFast) {
    ...
    if (tier === null || !Object.hasOwn(SPEED_TIERS, tier)) {
      reasons.push(reason('speed-signal-unproven', 'candidate'));
      unresolved = true;
    } else {
  ...
  if (unresolved) {
    return { status: 'unresolved', contribution: 0, reasons };
  }

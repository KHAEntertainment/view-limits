...
// in depth. `xsk-` is listed explicitly so the known embedded-token fixture
// stays scrubbed even though its `sk-` sits mid-word.
const SECRET_PREFIXES = [
  'sk-', 'tok-', 'ghp_', 'gho_', 'ghu_', 'ghs_', 'ghr_',
  'xoxp-', 'xoxa-', 'xoxb-', 'xoxs-', 'xsk-',
  'AKIA', 'ASIA',
];
// A known prefix only counts at a word boundary — start of string or after
// a non-alphanumeric char. Without the boundary, ordinary ids like
// 'task-runner', 'risk-eval', 'disk-cache', or 'desk-main' would be falsely
// scrubbed for containing 'sk-' mid-word. Non-global: no lastIndex state.
const SECRET_ID_RE = new RegExp(
  `(?<![a-zA-Z0-9])(?:${SECRET_PREFIXES.map((p) => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})`,
);
// JWT-shaped tokens (base64url three-segment). The `eyJ` prefix covers the
// standard `{"..."` header form — a bare JWT with no marker prefix still
// carries a segment structure no legitimate route id shares.
const JWT_RE = /eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]*/;
// Regex for token-shaped substrings in detail strings: known prefixes at a
// word boundary (continuing through the full token charset so dotted/base64
// suffixes are consumed whole rather than leaking `.def`/`+/=`-style
// tails), bare JWTs, and generic Bearer <token> patterns.
const SECRET_DETAIL_RE = new RegExp(
  `(?<![a-zA-Z0-9])(?:${SECRET_PREFIXES.map((p) => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})[a-zA-Z0-9_.+/=-]+|${JWT_RE.source}|Bearer\\s+\\S+`,
  'g',
);

// Reason codes this module emits — the reason-summaries completeness test
// imports this registry so a new literal without a summary fails the suite.
const RECEIPT_REASONS = Object.freeze({
  RECEIPT_ABSENT: 'receipt-absent',
});
...
// [#22] Atomic write: temp file + rename prevents torn reads from concurrent
// consumers (report/session-start can read while the server writes).
function writeReceipt(dataDir, { outcome, routeIds, detail }) {
  // NEVER throws — an invalid dataDir returns null like any other write
  // failure, before any path construction runs.
  if (!nonEmptyString(dataDir)) return null;
  ...
  // Boundary-aware prefix match — a token embedded at a word boundary
  // (`prod-ghp_abc`, `xsk-embedded`) must not survive into CLI output, but
  // an ordinary id containing 'sk-' mid-word (`task-runner`) is not token
  // material. A bare-JWT-shaped id is token material wherever it sits.
  const safeIds = routeIds.filter((id) => !SECRET_ID_RE.test(id) && !JWT_RE.test(id));
  ...
  const target = receiptPath(dataDir);
  ...
  try {
    ...
    // [#22] A failed write or rename must not leave the temp file behind.
...
function formatReceipt(receipt) {
  ...
  switch (r.outcome) {
    case OUTCOME_SUBMITTED:
      // [#16] A partial submission carries detail naming the routes that
      // still need credentials — it must reach the rendered message.
      return `credentials rotated for ${routes} at ${time}${r.detail ? ': ' + r.detail : ''}`;
...
function receiptFact(dataDir) {
  ...
  if (receipt) {
    return {
      value: {
        ...
        // [#16] Carry detail through so consumers rendering via
        // formatReceipt(receiptFact(...)) see the failure reason.
        detail: nonEmptyString(receipt.detail) ? receipt.detail : null,
      },
      provenance: 'observed',
  ...
  return {
    ...
    observedAt: null,
    freshUntil: null,
    reason: RECEIPT_REASONS.RECEIPT_ABSENT,
  };
}

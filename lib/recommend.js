...
// the suite.
const RECOMMEND_REASONS = Object.freeze({
  GATE_CLOSED: 'jev-readiness-gate-closed',
  MODEL_UNVERIFIED: 'jev-model-unverified',
  VERIFICATION_NOT_ATTEMPTED: 'jev-verification-not-attempted',
  DIMENSION_UNRESOLVED: 'profile-dimension-unresolved',
  SUGGESTION_UNKNOWN_CANDIDATE: 'jev-suggestion-unknown-candidate',
  SUGGESTION_INELIGIBLE: 'jev-suggestion-ineligible',
  ID_DUPLICATE: 'candidate-id-duplicate',
});
...
async function recommend({
  ...
  const verificationDetail = isPlainObject(verification)
    ...
    : (verification === true
      ...
      : { status: 'unverified',
          reason: itemsOnly.notPass.length === 0 ? RECOMMEND_REASONS.MODEL_UNVERIFIED : RECOMMEND_REASONS.VERIFICATION_NOT_ATTEMPTED });

  // ---- task profile: Jev when open+valid, deterministic fallback otherwise ---
  const jevSection = {
    attempted: false,
    status: 'dormant',
    reason: RECOMMEND_REASONS.GATE_CLOSED,
  ...
  if (itemsOnly.notPass.length === 0 && !gate.open) {
    ...
    jevSection.reason = RECOMMEND_REASONS.MODEL_UNVERIFIED;
  }
  ...
  candList.forEach((cand, i) => {
    ...
    if (duplicateIds.has(id)) {
      const reasons = [{
        code: RECOMMEND_REASONS.ID_DUPLICATE,
        scope: `candidate:${id}`,
  ...
  if (jevSuggestion !== null) {
    ...
    if (!verdict) {
      jevSection.suggestion = { candidateId: jevSuggestion, disposition: 'rejected', code: RECOMMEND_REASONS.SUGGESTION_UNKNOWN_CANDIDATE };
    } else if (verdict.result !== 'eligible') {
      jevSection.suggestion = {
        candidateId: jevSuggestion,
        disposition: 'rejected',
        code: RECOMMEND_REASONS.SUGGESTION_INELIGIBLE,
  ...
  for (const u of profile.unresolved) {
    unresolvedEvidence.push({ scope: `task-profile.${u.dimension}`, code: RECOMMEND_REASONS.DIMENSION_UNRESOLVED, reason: u.reason });
  }
...
module.exports = { recommend, RECOMMEND_SCHEMA_VERSION, RECOMMEND_REASONS };

...
//     harnessAvailable,         — boolean harness availability observation
...
//       usableRoute,            — truthy: route state must be proven observed
//     },
...
function candidateFamily(reg, modelEntryFact, declaredFamilyFact) {
  if (factIsKnown(modelEntryFact) && isPlainObject(modelEntryFact.value) && nonEmptyString(modelEntryFact.value.family)) {
    return norm(modelEntryFact.value.family);
  }
...
function subjectFamily(reg, subject) {
  ...
  if (factIsKnown(entry) && isPlainObject(entry.value) && nonEmptyString(entry.value.family)) {
    return norm(entry.value.family);
  }
  return null;
}
...
function evaluate(candidate, policy, registry) {
  ...
  const push = (list, code, scope) => {
    ...
    if (!list.some((r) => `${r.code} ${r.scope}` === key)) list.push({ code, scope, summary: REASON_SUMMARIES[code] });
  };
  ...
  if (forbidRoutes.length) {
    ...
    if (id === null) push(unresolved, 'route-identity-unproven', 'route');
    else if (forbidRoutes.includes(norm(id))) push(ineligible, 'route-forbidden', `route:${id}`);
  }
  ...
  const stateVal = knownString(routeState);
  if (stateVal !== null && norm(stateVal) === 'exhausted') {
    const id = knownString(routeId);
    push(ineligible, 'route-exhausted', `route:${id === null ? 'candidate' : id}`);
  }
  if (factIsKnown(harnessAvailable)) {
    const v = harnessAvailable.value;
    if (v === false || norm(v) === 'unavailable') {
  ...
  if (needRouteState && (stateVal === null || norm(stateVal) === 'unknown')) {
  ...
  // execution capabilities). Proven-false already produced ineligible above;
  // here unproven availability under a harness requirement stays unresolved.
  if ((reqHarnesses.length || reqExecCaps.length) && !factIsKnown(harnessAvailable)) {

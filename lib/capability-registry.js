...
//   * models     — task capabilities a model is configured to perform
...
// `family` reference.
function indexSection(section, kind, listFields) {
  const byId = new Map();
...
function createRegistry(entries) {
  ...
  const models = indexSection(doc.models, 'model', ['taskCapabilities']);

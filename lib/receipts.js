...
// (continuing through the full token charset so dotted/base64 suffixes are
// consumed whole rather than leaking `.def`/`+/=`-style tails), bare JWTs,
// and generic Bearer <token> patterns.
const SECRET_DETAIL_RE = new RegExp(
  `(?:${SECRET_PREFIXES.map((p) => p.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})[a-zA-Z0-9_.+/=-]+|${JWT_RE.source}|Bearer\\s+\\S+`,
  'g',
);
...
function writeReceipt(dataDir, { outcome, routeIds, detail }) {
  ...
  // Substring match — a token embedded mid-id (e.g. `prod-ghp_abc`) must
  // not survive into CLI output just because it isn't a prefix, and a
  // bare-JWT-shaped id is token material wherever it sits.
  const safeIds = routeIds.filter((id) =>
    !SECRET_PREFIXES.some((prefix) => id.includes(prefix)) && !JWT_RE.test(id));

...
function buildStatus({ exhausted = false, constrained = false, windows = [], balance = null, resetAt = null, detail = null }) {
  let state = 'healthy';
...
// unit is 'percent' (quota windows) or 'currency' (monetary credit/balance windows).
function window(type, remaining, limit, resetAt = null, unit = 'percent') {
  return { type, remaining, limit, resetAt, unit };
}
...
// Tolerant number parser (provider fields are often decimal strings).
function num(v) {

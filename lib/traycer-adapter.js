...
function boolFact(v, observedAt) {
  return typeof v === 'boolean' ? observed(v, observedAt) : unknown('traycer-field-unconforming');
}
...
function callerOverlay(row, agentId, epicId, observedAt) {
  const overlay = {
    ade: observed(ADE_ID, observedAt),
    agentId: observed(agentId, observedAt),
    epicId: observed(epicId, observedAt),
  };
  if (row) {
...
async function readTraycerRuntime({
  ...
  // will actually use — and therefore the identity every emitted row is
  // scoped to. A supplied value that disagrees with the env can never be
  // confirmed and is a caller mismatch, not a silently re-scoped read.
  ...
  if (agentsTask.result.ok) {
    ...
    if (!isPlainObject(data) || !isPlainObject(data.caller) || !Array.isArray(data.agents)) {
      ...
      // The local host is the caller's own host; fall back to any local row,
      // then to the supplied hostname.
      keyHost = (selfRow && selfRow.hostId) || localHostId || keyHost;
      if (invalid) pushDiag('traycer-output-malformed', 'sessions');
      if (verified && !mismatch) {
        if (selfRow) {
          out.caller = callerOverlay(selfRow, queryAgentId, queryEpicId, ts);
          callerSurface = nonEmptyString(selfRow.surface) ? selfRow.surface : null;
        } else {
          out.caller = callerOverlay(null, queryAgentId, queryEpicId, ts);
  ...
  for (const t of profileTasks) {
    ...
    for (const p of data.profiles) {
      ...
      if (p.isEffectiveLastUsed === true) {
        lastUsedReads.push({ harness: t.harness, selection: p.selection, row });
      }
    }
    if (invalid) pushDiag('traycer-output-malformed', t.scope);
  }

  // Wave 2 — fresh native rate limits, last-used profiles only.
  await pooled(lastUsedReads, concurrency, async ({ harness, selection, row }) => {
    const sel = selection.kind === 'ambient' ? 'ambient' : selection.profileId;
    const res = await invoke(['agent', 'profile-rate-limits', harness, '--profile', sel, '--json']);
    const scope = `rate-limits:${harness}`;

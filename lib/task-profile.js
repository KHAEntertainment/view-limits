...
function dimensionFact(input, vocab, { provenance, source }) {
  if (input === undefined || input === null) return unknown('task-profile-field-absent');
  if (isPlainObject(input)) {
    if (!('value' in input) || input.value === null || input.value === undefined ||
        input.provenance === 'unknown') {
      return unknown(nonEmptyString(input.reason) ? input.reason : 'task-profile-field-absent');
    }
    const n = nonEmptyString(input.value) ? input.value.trim().toLowerCase() : null;
    if (n === null || !vocab.includes(n)) return unknown('task-profile-value-invalid');
    return {
      value: n,
...
function buildProfile(layers, { source = 'task-profile' } = {}) {
  ...
  for (const name of DIMENSION_ORDER) {
    let f = unknown('task-profile-field-absent');
    for (const layer of inputs) {
      ...
      else if (!factIsKnown(f) && candidate.reason !== 'task-profile-field-absent') f = candidate;
    }
    dimensions[name] = f;
...
function validateClassifierResponse(raw) {
  ...
  if (!isPlainObject(raw)) {
    return { ok: false, violations: ['response-not-object'] };
  }
  const conf = raw.confidence;
  if (typeof conf !== 'number' || !Number.isFinite(conf) || conf < 0 || conf > 1) {
    violations.push('confidence-invalid');
  }
  const prof = raw.profile;
  if (!isPlainObject(prof)) {
    violations.push('profile-not-object');
  } else {
    for (const name of DIMENSION_ORDER) {
      const v = prof[name];
  ...
  if (raw.suggestedCandidateId !== undefined && !nonEmptyString(raw.suggestedCandidateId)) {
    violations.push('suggestedCandidateId-invalid');
  }
  violations.sort();
